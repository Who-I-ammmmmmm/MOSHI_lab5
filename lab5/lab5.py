...
class Lesson:
    def __init__(self, Class="None", teacher=None, name="None", special_room_required=False,
...
def generate_schedule(lesson_names, teachers, classes, num_lessons, num_classes, num_days,
    ...
    for i in range(num_classes):
        for j in range(num_lessons):
            ...
            if random_lesson[0] in classes[i].Class_teacher.lessons:
                random_teacher = classes[i].Class_teacher
            else:
                while True:
...
def fitness(schedule, num_lessons, max_lessons_per_day):
    ...
    for class_schedule in schedule:
        ...
        for day in class_schedule:
            for lesson in day:
                if lesson is not None:
                    if lesson.Class.Class_teacher.name == lesson.teacher.name:
                ...
                for lesson in day:
                    if lesson is not None:
                        if random.random() < mutation_rate and lesson is not None:
                            ...
                            if random_lesson[0] in lesson.Class.Class_teacher.lessons:
                                random_teacher = lesson.Class.Class_teacher
                            else:
                                while True:
                                    random_teacher = random.choice(teachers)
                                    if random_lesson[0] in random_teacher.lessons:
...
def print_schedule_to_excel(schedule, num_days, max_lessons_per_day, num_classes, class_names):
    ...
    # Запис заголовків
    headers = ["Day", "Lesson Number"] + class_names[:num_classes]
    ws.append(headers)
    ...
    for day_index in range(num_days):
        for lesson_index in range(max_lessons_per_day):
            ...
            for class_index in range(num_classes):
                if lesson_index >= len(schedule[class_index][day_index]):
                    row_data.append(" ")
                else:
                    lesson = schedule[class_index][day_index][lesson_index]
                    subject_teacher_room = f"{lesson.name} - ({lesson.teacher.name}) - ({lesson.room})"
                    row_data.append(subject_teacher_room)

            ws.append([f"Day {day_index + 1}", f"Lesson {lesson_index + 1}"] + row_data)

        if day_index < num_days - 1:
            ws.append([""] * (num_classes + 3))  # Додати порожній рядок між днями
...
def main():
    ...
    teachers = [
        Teacher("Teacher 1", ["Math", "English", "Physics", "Chemistry"]),
        Teacher("Teacher 2", ["Math", "English", "Physics", "Music"]),
        Teacher("Teacher 3", ["Math", "English", "Physics", "Chemistry"]),
    ...
    lesson_names = [["Math", False, "None"], ["English", False, "None"], ["Physics", False, "None"],
                    ["Chemistry", False, "None"], ["PE", True, "PE Room"], ["Choreography", True, "Dance Room"],
    ...
    classes = []
    for i in range(num_classes):
        classes.append(Class(class_names[i], teachers[i]))

    best_schedule = genetic_algorithm(lesson_names, teachers, classes, num_lessons, num_classes,
                                      num_days, max_lessons_per_day, room_names)
    # Виклик функції для створення файлу Excel з розкладом
    print_schedule_to_excel(best_schedule, num_days, max_lessons_per_day, num_classes, class_names)
